...
# Enable SSH and ARD remote access for northfayette user
...
# define script name
scriptName='Enable SSH and ARD remote access for northfayette user'
...
# setting up ARD for user northfayette
debugMessage "Setting ARD permissions for northfayette user"
/System/Library/CoreServices/RemoteManagement/ARDAgent.app/Contents/Resources/kickstart -activate -configure -access -on -users "northfayette" -privs -DeleteFiles -ControlObserve -TextMessages -OpenQuitApps -GenerateReports -RestartShutDown -SendFiles -ChangeSettings -clientopts -setmenuextra -menuextra no
...
# restrict ssh remote access to enable acces only for northfayette user
echo "AllowUsers northfayette" >> /etc/sshd_config
/usr/sbin/sshd reload

# add user to group
debugMessage "Adding northfayette to group com.apple.access_ssh..."
dseditgroup -o edit -n "/Local/Default" -a "northfayette" -t user com.apple.access_ssh
